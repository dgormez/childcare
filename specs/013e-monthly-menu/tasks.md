...
- [X] T062 Regenerate `web/lib/generated/api-types.ts` (depends on T061)
- [X] T063 [US4] Create `web/components/menu/MealPreferenceRequestQueue.tsx` — pending-request list, each item showing requested texture/dietary tags, the parent's note, the child's active health records, and Approve / Reject (with an optional reason field) actions (depends on T062)
- [X] T064 [US4] Wire `MealPreferenceRequestQueue` into `web/app/(app)/menu/page.tsx`, alongside the day-grid authoring section (depends on T023, T063)
- [X] T065 [P] [US4] Add `mealPreferenceRequests.*` i18n keys (queue labels, approve/reject actions, reason field, status badges — pairing icon with color per design-system.md) to `web/i18n/locales/{en,fr,nl}.json`
- [X] T066 [US4] Locate the existing `NotificationType`-to-i18n-key mapping that already renders `DayReservationDecided` and add the `MealPreferenceRequestDecided` case alongside it. Corrected premise: `web/` has no notifications screen at all (director-web never consumes parent-facing notifications) — this task's own text listing "web notifications screen and parent-mobile/app/(app)/notifications/" as both existing today was wrong; only `parent-mobile/app/(app)/notifications.tsx` needed the update (`ICON_BY_TYPE`, `NotificationType` union, `handlePress`'s switch → navigates to `/(app)/menu`, plus `parent.notifications.meal_preference_request_decided.*` i18n keys across en/fr/nl) (depends on T002)
- [X] T067 [P] [US4] Web component test: `MealPreferenceRequestQueue` renders pending requests with health-record context and calls the approve/reject handlers, in `web/__tests__/MealPreferenceRequestQueue.test.tsx`
...
- [X] T071 Extend `TenantMigrationRolloutTests`' schema-revert helper for the three new tables' FKs (`monthly_menu_days` before `monthly_menus` in FK-dependency order; `meal_preference_change_requests` independently, FK to `children`), following the same fix every migration-adding feature since 012a has needed (research.md R7), in `backend/ChildCare.Api.Tests/TenantMigrationRolloutTests.cs`. Also needed the identical fix in `LegacyVaccinationMigrationTests.cs` (confirms 013d/013g/013h's shipped-notes: any test that reverts a tenant schema to an earlier migration state needs this, not just the one file usually named).
- [X] T072 [P] Run through quickstart.md's four scenarios end-to-end locally (backend + web + parent-mobile) and fix any discrepancy found. Covered by the 18 TestContainers-backed integration tests (real PostgreSQL, real HTTP, real ASP.NET Core pipeline — MonthlyMenuTests.cs + MealPreferenceRequestTests.cs), which exercise all four scenarios' exact request/response flows. Additionally ran Scenario 1's director-side steps (org register, location create, menu upsert/publish) live against the actual local dev server (not the test host) to confirm the real dev config/migration/DI wiring works outside the test harness — genuinely caught nothing new (the earlier DI-registration bug T058 already fixed would have surfaced here too). Did not complete the parent-side steps live: this dev environment has a real SMTP host configured (not the log-fallback path `ParentTestSupport`'s test helper relies on), so extracting a parent-invitation token outside the test harness would require actual email delivery — out of scope for 013e and unrelated to this feature's own code, so not chased further; the parent-side flows remain fully covered by the integration tests instead.
- [X] T073 [P] Verify NL/FR/EN i18n key parity (no missing translation in any locale file touched by T025/T036/T050/T065) across `web/i18n/locales/` and `parent-mobile/i18n/locales/`
- [X] T074 [P] Verify FR-019: confirm no monthly-menu or meal-preference-request endpoint is reachable under any caregiver device-token authorization policy — review `MonthlyMenuEndpoints.cs`/`MealPreferenceRequestEndpoints.cs` for `DeviceOrStaffOrDirector`/`DeviceOrDirector`-style policies and confirm neither is used here. Confirmed: both files use only `DirectorOnly`/`ParentOnly`.
