...
- [X] T050 [P] Run quickstart.md's full validation sequence end-to-end (create → verify via 013g read endpoint → rename → reorder → deactivate → reactivate → non-admin denial → 013g regression suite) — run manually via curl against a locally-started API + local Postgres (not the automated test suite, which already covers the same sequence): create, tenant-read visibility, rename, deactivate (audit fields populated), tenant-read hiding, reactivate (audit fields cleared), reorder, and non-admin 403 on both GET and POST all confirmed. Along the way, confirmed `grant-platform-admin` (and the pre-existing `backfill-growth-check`/`migrate-tenants`) require `DOTNET_ENVIRONMENT` (not `ASPNETCORE_ENVIRONMENT`) to pick up `appsettings.Development.json` when run via `dotnet run --` — `Host.CreateApplicationBuilder` reads the former, `WebApplication.CreateBuilder` reads the latter; a pre-existing quirk of this CLI dispatch shape, not a defect introduced here, but worth remembering for any future local CLI-command testing in this codebase.
- [X] T051 [P] Design-compliance pass on `VaccineTypeManagementTable.tsx`/the new route against design-system.md (spacing scale, no nested cards, shared component reuse) and platform-rules.md (director-web density/keyboard-focus requirements) — no fixes needed: every spacing utility across the three new files maps to the 4/8/12/16/24/32 scale, no nested cards/gradients, every interactive element inherits `focus-visible:ring-2 focus-visible:ring-primary` (shared `Button`/`Input`/native `<select>` styled to match), the table inherits the shared `Table`/`TableRow`/`TableCell` components' `40px`/`8px`/`12px` density unmodified, and the row layout (clickable name + small action-button cluster) directly mirrors `WaitingListTable`'s own already-shipped precedent rather than introducing a new pattern.
