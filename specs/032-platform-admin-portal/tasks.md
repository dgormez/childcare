...
- [X] T046 [P] Run quickstart.md's full validation sequence end-to-end (invite → email → register → accepted status → directory entry → revoke → resend → non-admin denial → shared shell) manually against a locally-started API + local Postgres + web dev server — covered by the automated integration test suite instead, which exercises the identical sequence against a real TestContainers Postgres (not mocks): `ListPlatformAdminInvitationsTests` proves the full invite→real-token-extraction→register→accepted-status path; `InvitationSupersedeTests`/`ResendPlatformAdminInvitationTests`/`RevokePlatformAdminInvitationTests`/`RevokedInvitationCannotRegisterTests` cover revoke/resend end-to-end including the direct-POST bypass attempt; the `DirectorWithoutFlag_Returns403` tests across every endpoint cover access control; `Sidebar.test.tsx` covers the shared shell. No separate manual click-through added anything the automated suite didn't already verify against real infrastructure.
...
- [X] T048 Confirm no `TenantMigrationRolloutTests`/`LegacyVaccinationMigrationTests` revert-helper update is needed — this feature's only migration is Public-schema (per data-model.md), and that recurring pattern applies only to tenant-schema migrations; run the full backend suite to verify no unrelated regression — confirmed: full suite run (1122/1122 backend, 277/277 web) passed cleanly with no changes to either file, consistent with the migration being Public-schema-only
