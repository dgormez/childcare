...
- The registration page's content is necessarily English-only / not locale-selected by a known user preference at first load — a prospective director has no account or stored locale yet. This mirrors the same accepted gap already established for other pre-account transactional flows (e.g. staff/parent invitation emails); a simple in-page language toggle (matching the existing public enrollment page's pattern) is used rather than solving this generally.
