    ...
    : IClassFixture<OrganisationOnboardingWebAppFactory>
{
    ...
    /// (FamilyGroupId), no new table, so it only needs its own history-removal entry. Feature
    /// 021's "AddLocationQrCheckInEnabled" migration is the same class of gap once more — one
    /// column on the already-dropped locations table (QrCheckInEnabled), no new table, so it
    /// only needs its own history-removal entry. Feature 023's "AddDigitalEnrollment" migration
    /// is the same class of gap again — three columns on the already-dropped locations table
    /// (DefaultEnrollmentLocale, PublicEnrollmentEnabled, PublicEnrollmentSlug) and seven on the
    /// already-dropped waiting_list_entries table (ReferenceCode, Source, SubmittedLocale,
    /// TourInvitationSentAt, TourInvitationStatus, TourOutcome, TourProposedAt), no new table, so
    /// it only needs its own history-removal entry. Feature 025's "AddCodaPaymentMatching"
    /// migration adds two new tables — coda_transactions has FKs to both coda_imports and
    /// invoices, so it must drop before either; coda_imports has no FK of its own, so it only
    /// needs to precede nothing further — same recurring pattern every migration-adding feature
    /// since 012a has needed. Feature 026's "AddSepaDirectDebit" migration adds one new table
    /// (sepa_batches, referenced by invoices.SepaBatchId) and four new columns (SepaBatchId/
    /// SepaMandateReferenceUsed/SepaReturnReason on invoices, SepaRevokedAt on contracts) — the
    /// four columns need no separate DROP COLUMN step since invoices/contracts are dropped
    /// wholesale below, but sepa_batches (the referenced table) must drop after invoices (the
    /// referencing table), same FK-direction rule 013g's shipped-note first flagged. Feature
    /// 027's "AddStaffAppPersonalRotaAndLeave" migration adds one new table (staff_leave_requests,
    /// FK to staff_profiles, so it must drop before staff_profiles/staff_schedules below) and six
    ...
    private static async Task<bool> HasExtensionColumnsAsync(IServiceProvider services, string schemaName)
    {
        ...
        try
        {
            ...
            _ = await db.CodaImports.CountAsync();
            _ = await db.CodaTransactions.CountAsync();
            _ = await db.SepaBatches.CountAsync();
            _ = await db.Invoices.Select(i => i.SepaBatchId).FirstOrDefaultAsync();
            _ = await db.Contracts.Select(c => c.SepaRevokedAt).FirstOrDefaultAsync();
            return true;
        }
        catch
        {
